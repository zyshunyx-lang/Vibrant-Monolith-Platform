...
export const DayDetailModal: React.FC<DayDetailModalProps> = ({ isOpen, onClose, dateStr, onUpdate }) => {
  ...
  const executeOperation = () => {
    if (!targetSlot || !selectedNewUserId || !reason || !currentUser) return;
  ...
  const getCandidates = () => {
    ...
    if (activeAction === 'standby') {
      ...
      return getStandbyCandidates(dateStr, catId, dutyData, users).map(u => ({ label: `[Recommended] ${u.realName}`, value: u.id }));
    }
    return users.filter(u => u.isActive && u.id !== targetSlot.userId).map(u => ({ label: u.realName, value: u.id }));
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`${dateStr} Shift Details`} footer={<Button variant="secondary" onClick={onClose}>Close</Button>}>
      <div className="space-y-6">
          ...
          {schedule?.slots.map(slot => {
            ...
            return (
              <div key={slot.slotId} className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between">
                <div>
                  <div className="text-[10px] font-black text-indigo-500 uppercase">{slotDef?.name}</div>
        ...
        {dayLogs.length > 0 && (
          <div className="space-y-3">
             <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Audit Trail (Today)</h4>
                 ...
                 <div key={log.id} className="p-3 text-[11px] flex flex-col gap-1">
                   <div className="flex justify-between items-center">
                     <div className="flex items-center gap-2">
                       <Badge variant={log.type === 'swap' ? 'warning' : 'danger'}>{log.type.toUpperCase()}</Badge>
                   ...
                   <div className="text-slate-500 italic">Reason: {log.reason}</div>
                 </div>
               ))}
             </div>
          </div>
        ...
        {/* Action Sub-modal/Form */}
        <Modal isOpen={activeAction !== null} onClose={() => setActiveAction(null)} title={activeAction === 'swap' ? 'Personnel Swap' : 'Standby Assignment'}>
           <div className="space-y-4">
              <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100">
                <p className="text-xs font-medium text-indigo-800">Replacing <span className="font-bold">{getUserName(targetSlot?.userId || '')}</span> for {dateStr}</p>
              </div>
              <Select 
                label="New Personnel" 
                options={[{label: '-- Select Person --', value: ''}, ...getCandidates()]} 
                value={selectedNewUserId} 
                onChange={e => setSelectedNewUserId(e.target.value)}
              />
              <Input label="Reason" placeholder="e.g. Medical leave, Business trip..." value={reason} onChange={e => setReason(e.target.value)} />
              <div className="flex gap-2 justify-end pt-4">
                <Button variant="secondary" onClick={() => setActiveAction(null)}>Cancel</Button>
                <Button onClick={executeOperation} disabled={!selectedNewUserId || !reason}>Execute Change</Button>
              </div>
           </div>
        </Modal>
      </div>
